...
vector<GPUInfo> get_gpu_info() {
    ...
    while (fgets(buffer, sizeof(buffer), pipe)) {
        ...
        if (getline(iss, idx_str, ',') && getline(iss, free_mem_str, ',') && getline(iss, total_mem_str)) {
            GPUInfo gpu;
...
void list_idle_gpus(int min_idle_gb, bool sort_by_memory, bool to_json) {
    ...
    for (vector<GPUInfo>::const_iterator it = gpus.begin(); it != gpus.end(); ++it) {
    ...
    if (sort_by_memory) {
        sort(filtered.begin(), filtered.end(), [](const GPUInfo &a, const GPUInfo &b) {
    ...
    if (to_json) {
        ...
        for (size_t i = 0; i < filtered.size(); ++i) {
            cout << "{\"index\": " << filtered[i].index << ", \"memory_free_mb\": " << filtered[i].memory_free_mb << "}";
        ...
        for (vector<GPUInfo>::const_iterator it = filtered.begin(); it != filtered.end(); ++it) {
            const GPUInfo &gpu = *it;
            cout << "cuda:" << gpu.index << " " << gpu.memory_free_mb << " MB free" << endl;
        }
    }
}
...
void list_busy_gpus(bool sort_by_memory, bool to_json) {
    ...
    if (to_json) {
        ...
        for (size_t i = 0; i < busy_gpus.size(); ++i) {
            cout << "{\"index\": " << busy_gpus[i].index << ", \"memory_free_mb\": " << busy_gpus[i].memory_free_mb << "}";
        ...
        for (vector<GPUInfo>::const_iterator it = busy_gpus.begin(); it != busy_gpus.end(); ++it) {
            const GPUInfo &gpu = *it;    
            cout << "cuda:" << gpu.index << " " << gpu.memory_free_mb << " MB free" << endl;
        }
    }
...
void select_idle_gpu(int min_idle_gb, bool export_env, bool yaml_inject, optional<string> yaml_path, optional<string> yaml_key) {
    ...
    if (eligible.empty()) {
        cerr << "No idle GPU found with at least " << min_idle_gb << " GB free." << endl;
        exit(1);
    }
